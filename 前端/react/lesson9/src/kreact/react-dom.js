...
function placeChild(newFiber, lastPlaceIndex, newIdx, shouldTrackSideEffects) {
  ...
  //洁面更新阶段，但是这里的fiber有可能是新增的或者更新的，需要区分
  let base = newFiber.base
  if (base !== null) {
    // 复用olderFiber
    let oldIndex = base.index;
    if (oldIndex < lastIndex) {//改变
      return lastIndex
    } else { // 没改变
      return oldIndex
    }
...
function reconcileChildren(returnFiber, newChildren) {
  ...
  if (newIdx === newChildren.length) {
    ...
    // exitingChildren.forEach(child => deletions.push({ ...child, effectTag: DELETION }))
  }
  ...
  for (; newIdx < newChildren.length; newIdx++) {
    ...
    if (matchedFiber) {
      ...
      newFiber = {
        ...
        base: matchedFiber.base,
        effectTag: UPDATE
      }
      // 找到就要删除链表上的元素，防止重复查找
      shouldTrackSideEffects && existingChildren.delete(newFiber.key === null ? null : newFiber.key)
    } else {
      newFiber = {
        ...newFiber,
        node: null,
...
//暗号：几内亚//暗号：几内亚//暗号：几内亚//暗号：几内亚//暗号：几内亚//暗号：几内亚
